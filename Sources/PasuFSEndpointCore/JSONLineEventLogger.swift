...
public enum JSONLineEventLoggerError: Error, CustomStringConvertible, Sendable {
  ...
  public var description: String {
    switch self {
    case .recordTooLarge: "The audit record exceeds the log file size limit."
...
public final class JSONLineEventLogger: EndpointEventSink, @unchecked Sendable {
  ...
  public init(
    ...
    self.queue = DispatchQueue(label: "com.example.pasu.fs.endpoint-audit-log")
  ...
  private func write(_ event: EndpointEventRecord) {
    do {
      ...
      if data.count + 1 > maximumFileSize {
        data = try encoder.encode(Self.recordWithinLimit(event, maximumBytes: maximumFileSize))
      }
      data.append(0x0A)
  ...
  static func recordWithinLimit(_ event: EndpointEventRecord, maximumBytes: Int64) throws
    -> EndpointEventRecord
  {
    let encoder = JSONEncoder()
    encoder.dateEncodingStrategy = .iso8601
    guard try encoder.encode(event).count + 1 > maximumBytes else { return event }
    guard var lineage = event.processLineage else { throw JSONLineEventLoggerError.recordTooLarge }
    var bounded = event
    lineage.processes = []
    lineage.relations = []
    lineage.issues = [LineageIssue(reason: "oversizedRecord", at: lineage.capturedAt)]
    bounded.processLineage = lineage
    guard try encoder.encode(bounded).count + 1 <= maximumBytes else {
      throw JSONLineEventLoggerError.recordTooLarge
    }
    return bounded
  }
