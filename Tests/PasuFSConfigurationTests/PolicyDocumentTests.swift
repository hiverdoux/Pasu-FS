...
final class PolicyDocumentTests: XCTestCase {
  ...
  func testProtectionWhitelistAllowsEmptyAndDisabledRules() throws {
    ...
    for rules in [whitelist.rules, []] {
      whitelist.rules = rules
      let document = makeDocument(policies: [whitelist])
      XCTAssertNoThrow(try document.validate())
      let encoded = try PolicySetDocumentCodec.encode(document)
      XCTAssertEqual(try PolicySetDocumentCodec.decode(encoded), document)
    }
  ...
  func testHandshakeRequestCarriesCurrentProtocolVersion() throws {
    let request = XPCHandshakeRequest(
      nonce: Data([4, 5, 6])
    )
    let decoded = try JSONDecoder().decode(
  ...
  func testHandshakeResponseCarriesCatalogDigestWithoutExposingCatalogVersion() throws {
    let digest = String(repeating: "b", count: 64)
    let response = XPCHandshakeResponse(
      nonce: Data([7, 8, 9]),
      runtimeInstanceIdentifier: uuid(8),
      systemCompatibilityCatalogDigest: digest,
    ...
    XCTAssertEqual(decoded.systemCompatibilityCatalogDigest, digest)
  }
